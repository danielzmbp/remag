...
def parse_fasta(
    fasta_file: str, contig_filter: Optional[Set[str]] = None
) -> Dict[str, int]:
    """
            ...
            for line in f:
                ...
                if line.startswith(">"):
                    ...
                    if current_header is not None and (
                        contig_filter is None or current_header in contig_filter
                    ):
...
def calculate_coverage_batch(
    args: Tuple[List[Tuple[str, int]], str],
) -> Dict[str, float]:
    """
        ...
        with pysam.AlignmentFile(bam_file, "rb") as bamfile:
            ...
            for contig_name, contig_length in contig_batch:
                ...
                try:
                    ...
                    mean_coverage = (
                        np.mean(total_coverage_per_base)
                        if len(total_coverage_per_base) > 0
                        else 0.0
                    )
...
def calculate_coverage_fast_approximate(
    bam_file: str, contigs: Dict[str, int]
) -> Dict[str, float]:
    """
...
def calculate_coverage_for_bam(
    bam_file: str,
    ...
    if threads > 1 and len(batches) > 1:
        ...
        batch_results = [
            ...
            for batch_args in tqdm(
                batches, desc=f"Processing {os.path.basename(bam_file)}"
            )
        ]
...
def main():
    ...
    parser.add_argument(
        "-f", "--fasta", required=True, help="Input FASTA file (can be gzipped)"
    )

    parser.add_argument(
        "-b", "--bam", nargs="+", required=True, help="Input BAM file(s)"
    )
    ...
    parser.add_argument(
        "--min-length", type=int, default=1000, help="Minimum contig length to include"
    )
    ...
    if args.min_length > 0:
        ...
        logger.info(
            f"Filtered to {len(filtered_contigs)} contigs >= {args.min_length} bp"
        )
    ...
    result_df = df.pivot(
        index=["contig", "length"], columns="sample", values="coverage"
    )
