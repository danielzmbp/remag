...
class TestClusteringIntegration:
    ...
    def test_end_to_end_clustering_pipeline(
        ...
        try:
            ...
            clusters_df = cluster_contigs(
                sample_embeddings_df, sample_fragments_dict, gene_mappings, mock_args
            )
            ...
            assert all(
                isinstance(cluster_id, str) for cluster_id in clusters_df["cluster"]
            )
    ...
    def test_clustering_with_single_contig(self, mock_args):
        ...
        single_fragments = {"single_contig": {"sequence": "ATCGATCG", "length": 8}}

        from remag.clustering import cluster_contigs

        clusters_df = cluster_contigs(
            single_embedding_df, single_fragments, {}, mock_args
        )
...
class TestGraphCaching:
    ...
    def test_graph_caching_saves_and_loads(
        self, sample_embeddings_df, mock_args, temp_dir
    ):
