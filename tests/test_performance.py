...
from remag.clustering import (
    _permutation_anova_chimera_test,
    _vectorized_pairwise_distances,
)
...
class TestChimeraDetectionPerformance:
    ...
    def test_vectorized_vs_manual_distance_calculation(self):
        ...
        assert np.allclose(
            manual_distances, vectorized_distances, rtol=1e-10, atol=1e-10
        )
    ...
    def test_inter_group_distance_performance(self):
        ...
        # Vectorized calculation (new method)
        start_time = time.time()
        vectorized_inter_distances = _vectorized_pairwise_distances(
        ...
        assert np.allclose(
            manual_inter_distances, vectorized_inter_distances, rtol=1e-10, atol=1e-10
        )
    ...
    def test_vectorized_function_edge_cases(self):
        ...
        two_embeddings = two_embeddings / np.linalg.norm(
            two_embeddings, axis=1, keepdims=True
        )
...
class TestComplexityAnalysis:
    ...
    def test_correctness_with_known_vectors(self):
        ...
        for i, (actual, expected) in enumerate(zip(distances, expected_distances)):
            ...
            ), f"Distance {i}: expected {expected}, got {actual}"
...
class TestBenchmarks:
    ...
    def test_baseline_chimera_detection_benchmark(self):
        ...
        assert duration < 15.0, f"Baseline chimera detection too slow: {duration:.4f}s"
